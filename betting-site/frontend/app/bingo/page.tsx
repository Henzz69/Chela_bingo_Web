...
const STAKE_OPTIONS = [
  { label: '10 ETB', amount: 10, players: 2, prize: 18 },
  { label: '25 ETB', amount: 25, players: 4, prize: 90 },
  { label: '50 ETB', amount: 50, players: 6, prize: 270 },
  { label: '100 ETB', amount: 100, players: 8, prize: 720 },
];

// 🚀 Added 'home' back to the primary position
const NAV_TABS = [
      ...
      <AnimatePresence mode="wait">
        {screen === 'lobby' && (
          <motion.div key="lobby" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.25 }} className="flex-1 flex flex-col">
            ...
            <div className="px-4 py-6 text-center">
              ...
              <p className="text-white/50 text-sm">75-Ball Multiplayer Bingo</p>
            </div>
        ...
        {screen === 'select' && (
          <motion.div key="select" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.25 }} className="flex-1 flex flex-col">
            ...
            <div className="px-4 space-y-3 pb-24 overflow-y-auto max-h-[80vh]">
              {STAKE_OPTIONS.map(opt => {
                ...
                return (
                  <motion.button
                    ...
                    className="w-full bg-gradient-to-br from-[#0a4a2e] to-[#042b1a] border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.3)] backdrop-blur-md overflow-hidden relative rounded-2xl p-4 text-left disabled:opacity-50 transition-all duration-200"
                  >
                    <div className="relative z-10 flex items-center justify-between">
                      <div>
                        <p className="text-white font-bold text-base">{opt.label} Room</p>
                        <p className="text-white/50 text-xs mt-0.5">Max {opt.players} players</p>
                      </div>
                      <div className="text-right">
                        <p className="text-green-400 font-bold text-sm">
                          {isThisLoading ? '⌛' : `🏆 ${opt.prize} ETB`}
                        </p>
                      </div>
                    </div>
                    {isThisLoading && <div className="absolute inset-0 bg-[#0a4a2e]/80 flex items-center justify-center"><div className="w-5 h-5 border-2 border-green-400 border-t-transparent rounded-full animate-spin"></div></div>}
                  </motion.button>
                );
              })}
            </div>
